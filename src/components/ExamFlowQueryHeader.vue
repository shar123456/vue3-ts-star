<template>
    ...
    <div class="upPad">
      <div class="upPad-up">
        ...
        <div>
          ...
          <div>
            <a-input
              v-model:value="QueryConditionInfo.flowNo"
              placeholder="流程编号："
            />
          </div>     
        </div>
 ...
 <div>
          <label>流程名称：</label>
          <div>
             <a-input v-model:value="QueryConditionInfo.flowName" placeholder="流程名称：" />
          </div>     
        </div>
...
export default defineComponent({
  ...
  props: {  },
  setup(props, context) {
    const state = reactive(new ExaminationFlowEntity());
    const SearchBtn = () => {
    ...
    const ClearQueryBtn = () => {
      ...
      state.QueryConditionInfo.flowName = "";
